 ...
 __device__ Network::Network(int *dim_lay, int dim_size, Funct *f, Funct *l, DevData *train, cublasHandle_t *hdl, int maxDimen)
{
 ...
 __device__ void Network::sethandle(cublasHandle_t * hdl)
{
	Layer *curn_lay_ptr = head_lay_ptr;
	curn_lay_ptr->handle = hdl;
...
__device__ void Network::predict(DevData &a, float * labels)
{
	for(int i =0; i < a.nrow(); i++)
	{
		labels[i] = predictInner(a, i);
	}	
}

__device__ float Network::predictInner(DevData &a, int obs_id)
{
	printf("now we're here\n");
	Layer *curn_lay_ptr = head_lay_ptr;
	curn_lay_ptr->push(obs_id, &a);
	curn_lay_ptr = curn_lay_ptr->next();
