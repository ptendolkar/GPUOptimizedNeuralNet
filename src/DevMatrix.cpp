...
__device__ void DevMatrix::copy(DevMatrix &X, cublasHandle_t *hdl){
...
__device__ void saxpy(cublasHandle_t *hdl, const float alpha, DevMatrix &x, const int inc_x, DevMatrix &y, const int inc_y)
{
	const int n = y.size();
...
__device__ void saxpy(cublasHandle_t *hdl, float alpha, const float &x, const int inc_x, DevMatrix &y, const int inc_y)
{
	const int n = y.nrow();

	cublasSaxpy(*hdl, n, &alpha, &x, inc_x, y.begin(), inc_y);
	//__syncthreads();
 	//printf("rf %d info %d\n", status);
}

__device__ void sgemv(cublasHandle_t *hdl, cublasOperation_t trans, const float alpha, DevMatrix &A, DevMatrix &x, int inc_x, const float beta, DevMatrix &y, const int inc_y)
{
	int M = A.nrow();
	int N = A.ncol();
...
__device__ void sgemv(cublasHandle_t *hdl, cublasOperation_t trans, const float alpha, DevMatrix &A, const float &x, const int inc_x, const float beta, DevMatrix &y, const int inc_y)
{
	int M = A.nrow();
	int N = A.ncol();
...
__device__ void ssbmv(cublasHandle_t *hdl, cublasFillMode_t uplo, const float alpha, DevMatrix &A, const int K, DevMatrix &x, const int inc_x, const float beta, DevMatrix &y, const int inc_y)
{
...
__device__ void sger(cublasHandle_t *hdl, const float alpha, DevMatrix &x, const int inc_x, const float &y, const int inc_y, DevMatrix &A)
{
	int M  = A.nrow();
	int N  = A.ncol();
...
//A := alpha*x*y**T + A 
__device__ void sger(cublasHandle_t *hdl, const float alpha, DevMatrix  &x, const int inc_x, DevMatrix &y, const int inc_y, DevMatrix &A)
{
	int M  = A.nrow();
	int N  = A.ncol();
...
__device__ void sgemm(cublasHandle_t *hdl, cublasOperation_t TrA, cublasOperation_t TrB, float alpha, DevMatrix &A, DevMatrix &B, float beta, DevMatrix &C)
